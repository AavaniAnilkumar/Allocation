...
//     try {
//       await axios.post(`${BASE_URL}/api/userrole-permission/${userRoleId}/${permissionId}/`,null,{
//         headers: {
//           Authorization: `Bearer ${localStorage.getItem('accessToken')}`,
//         },
//       });
...
//     try {
//       await axios.delete(`${BASE_URL}/api/userrole-permission/${userRoleId}/${permissionId}/`,{
//         headers: {
//           Authorization: `Bearer ${localStorage.getItem('accessToken')}`,
//         },
//       });
...
const Permissions = () => {
  ...
  const fetchTimeCyclePermissions = async () => {
    try {
      ...
      // Assuming the response structure includes codename and permissionId
      const timeCyclePermissionsWithData = await Promise.all(
        response.data.time_cycle_permissions.map(async (permission) => {
          try {
            const permissionDataResponse = await axios.get(`${BASE_URL}/api/permissions/${permission.permissionId}/`, {
              headers: {
                Authorization: `Bearer ${localStorage.getItem('accessToken')}`,
              },
            });
            return {
              ...permission,
              name: permissionDataResponse.data.name,
            };
          } catch (error) {
            console.error(`Error fetching permission data for ${permission.permissionId}:`, error);
            return permission;
          }
        })
      );
  
      setTimeCyclePermissions(timeCyclePermissionsWithData);
    } catch (error) {
      console.error('Error fetching time cycle permissions:', error);
    }
  };
  ...
  const handleAddPermission = async (permissionId, userRoleId) => {
    try {
      ...
      setSelectedUserRole(userRoles.find(role => role.id === userRoleId)?.name || '');
    }
  };
