...
function AllocationTable() {
  ...
  const [loadingAllocation, setLoadingAllocations ] =useState(false);
  ...
  const handleCreateAllocation = () => {
    if (selectedTimeCycle) {
      setLoadingAllocations(true); // Set loading state
      ...
      fetch(`${BASE_URL}/api/timecycle/${selectedTimeCycle.id}/allocate/`, {
        ...
        .then((data) => {
          // Handle successful creation
          console.log('Allocation created:', data);
          // Update the selected time cycle state
          setSelectedTimeCycle((prevSelectedTimeCycle) => ({
            ...prevSelectedTimeCycle,
            allocations: data, // Assuming the API response contains allocations
          }));
          // Fetch updated allocations for the selected time cycle
          fetchAllocations(selectedTimeCycle.id);
        })
        .catch((error) => console.error('Error creating allocation:', error))
        .finally(() => setLoadingAllocations(false)); // Reset loading state
    }
  };
...
const renderOffDayList = () => {
  ...
  const fetchAllocations = (timeCycleId) => {
    // Fetch allocation data for the selected time cycle from Django backend API
    fetch(`${BASE_URL}/api/allocations/${timeCycleId}/`, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem('accessToken')}`,
      },
    })
          ...
          {timeCycles.map((timeCycle) => (
            <tr key={timeCycle.id}>
              <td>
                <strong>
                  <span
                    ...
                    onClick={() => handleViewAllocations(timeCycle)}
                  >
                    {timeCycle.time_cycle_name}
                  </span>
                </strong>
                <br />
                {timeCycle.start_date} to {timeCycle.end_date}
              ...
              <td>
                ...
                <Button
                  variant="success"
                  onClick={() => handleCreateAllocation(timeCycle)}
                >
                  Create Allocation Table
                </Button>
              </td>
    ...
    return (
      ...
      {allocations.length > 0 &&
        allocations[0].days.map((dayAllocation, index) => (
          <th key={index} style={{whiteSpace: 'nowrap'}}>{dayAllocation.date}</th>
        ))}
           </tr>
         </thead>
         <tbody>
  ...
  return (
    <div className='allocation-container'>
      ...
      <div className='allocation-table'>
        ...
        {selectedTimeCycle && (
        <div>
          <h2 style={{textAlign: 'center', fontFamily:'cursive'}}> {selectedTimeCycle.time_cycle_name}</h2>
        </div>
      )}
          {renderTable()}
